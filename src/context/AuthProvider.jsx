...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => { //Efecto que verifica que verifica si hay data del localstorage,por medio del stGetAuth
    const {data, success} = stGetAuth(); 
    if (success && data ){ //Si hay data y success es true
      const { isLogin, ...payload } = data;
      authDispatch({ type: typesGlobalState.authLogin, payload });
    }else{ //Si no hay data se dipara el dispatch y se desloguea.
